  ...
  private void createQuiz()
{
  try
  {
    ...
    java.util.Scanner file = new java.util.Scanner(new File("quiz.txt"));
  ...
  private void newPlayerLevel()
{
  try
  {
    java.util.Scanner file = new java.util.Scanner(new File("level.txt"));
    
    if (file.hasNextInt())
    {
      playerLevel = file.nextInt();
    }
    file.close();

    // increase walls as level increases
    numWalls = 5 + playerLevel;
    ...
    if (e.getKeyCode() == KeyEvent.VK_P)
{
  if (atPrize)
  {
    String question = quiz[currentQuestion][0] + "\n"
    ...
    if (userAnswer != null &&
        userAnswer.trim().equalsIgnoreCase(quiz[currentQuestion][5]))
    {
      showMessage("Correct!");
      score += correctAns;
      pickupPrize();
    ...
    if (e.getKeyCode() == KeyEvent.VK_Q)
{
  ...
  if (!allCollected)
  {
    showMessage("You must collect all prizes first!");
    return;
  }
  ...
  if (score > 0 && playerLevel < MAX_LEVEL)
  {
    playerLevel++;
  }
